...
export class DashboardAnalyticsService {
  ...
  async getAttackRatio(): Promise<AttackRatioStats> {
    try {
      ...
      const clampedAttackRequests = Math.min(attackRequests, totalRequests);
      const normalRequests = Math.max(totalRequests - clampedAttackRequests, 0);
      const attackPercentage = totalRequests > 0
        ? (clampedAttackRequests / totalRequests) * 100
        : 0;

      return {
        totalRequests,
        attackRequests: clampedAttackRequests,
        normalRequests,
