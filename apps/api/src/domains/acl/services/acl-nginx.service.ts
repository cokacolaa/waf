...
export class AclNginxService {
  ...
  private readonly ACL_RATE_LIMIT_CONFIG_FILE = '/etc/nginx/conf.d/acl-rate-limits.conf';
  private readonly NGINX_TEST_CMD = 'nginx -t';
  private readonly NGINX_RELOAD_CMD = 'nginx -s reload';
  private readonly RATE_LIMIT_ZONE_SIZE = '10m';
  private readonly RATE_LIMIT_KEY = '$binary_remote_addr';

  /**
   * Generate Nginx ACL configuration from database rules
   */
  async generateAclConfig(rules?: AclRuleEntity[]): Promise<string> {
    try {
      // Get all enabled ACL rules
      const enabledRules = rules ?? await aclRepository.findEnabled();
...
\n`;
      ...
      const ipRules = enabledRules.filter(r => r.conditionField === 'ip');
      const userAgentRules = enabledRules.filter(r => r.conditionField === 'user_agent');
      const geoipRules = enabledRules.filter(r => r.conditionField === 'geoip');
      const urlRules = enabledRules.filter(r => r.conditionField === 'url');
      const methodRules = enabledRules.filter(r => r.conditionField === 'method');
      const headerRules = enabledRules.filter(r => r.conditionField === 'header');
      const rateLimitRules = enabledRules.filter(r => r.conditionField === 'rate_limit');
      ...
      // Generate GeoIP rules
      if (geoipRules.length > 0) {
        config += `\n# ===== GeoIP-Based Access Control =====\n\n`;
        config += `# Requires http_geoip_module (uses $geoip_country_code)\n\n`;

        const geoWhitelists = geoipRules.filter(r => r.type === 'whitelist');
        const geoBlacklists = geoipRules.filter(r => r.type === 'blacklist');

        if (geoWhitelists.length > 0) {
          config += `# GeoIP Whitelists (Allow)\n`;
          config += `set $acl_geoip_whitelisted 0;\n`;
          for (const rule of geoWhitelists) {
            const condition = this.buildGeoipCondition(rule);
            if (!condition) {
              continue;
            }
            config += `# ${rule.name}\n`;
            config += `if (${condition}) {\n`;
            config += `    set $acl_geoip_whitelisted 1;\n`;
            config += `}\n`;
          }
          config += `\n`;
        }

        if (geoBlacklists.length > 0) {
          config += `# GeoIP Blacklists (Deny)\n`;
          for (const rule of geoBlacklists) {
            const condition = this.buildGeoipCondition(rule);
            if (!condition) {
              continue;
            }
            if (rule.action !== 'deny' && rule.action !== 'challenge') {
              continue;
            }
            config += `# ${rule.name}\n`;
            if (geoWhitelists.length > 0) {
              config += `if ($acl_geoip_whitelisted = 0) {\n`;
              config += `    if (${condition}) {\n`;
              if (rule.action === 'deny') {
                config += `        return 403 "Access Denied";\n`;
              } else if (rule.action === 'challenge') {
        ...
        if (geoWhitelists.length > 0 && geoBlacklists.length === 0) {
          config += `# Deny all countries not explicitly whitelisted\n`;
          config += `if ($acl_geoip_whitelisted = 0) {\n`;
          config += `    return 403 "Access Denied";\n`;
          config += `}\n`;
        }
      ...
      if (rateLimitRules.length > 0) {
        ...
        config += `# Requires ${this.ACL_RATE_LIMIT_CONFIG_FILE} included in http context\n\n`;
        const rateLimitStatus = this.getRateLimitStatus(rateLimitRules);
        if (rateLimitStatus) {
          config += `limit_req_status ${rateLimitStatus};\n\n`;
        }
        for (const rule of rateLimitRules) {
          ...
          const zoneName = this.buildRateLimitZoneName(rule);

          config += `# ${rule.name}\n`;
          config += `# Rate: ${rule.conditionValue}\n`;
          config += `limit_req zone=${zoneName} burst=20 nodelay;\n\n`;
        }
      }
  ...
  /**
   * Build GeoIP match condition for nginx if statements
   */
  private buildGeoipCondition(rule: AclRuleEntity): string | null {
    const rawValue = rule.conditionValue?.trim();
    if (!rawValue) {
      return null;
    }

    if (rule.conditionOperator === 'equals') {
      return `$geoip_country_code = "${rawValue}"`;
    }

    if (rule.conditionOperator === 'regex') {
      return `$geoip_country_code ~* "${rawValue}"`;
    }
    ...
    if (rule.conditionOperator === 'contains') {
      ...
      if (codes.length === 1) {
        return `$geoip_country_code = "${codes[0]}"`;
      }
      const escaped = codes.map(code => code.replace(/[\\.^$|?*+()[\]{}]/g, '\\$&'));
      return `$geoip_country_code ~* "^(?:${escaped.join('|')})$"`;
    }
  ...
  /**
   * Generate rate limit zones (http context only)
   */
  async generateRateLimitConfig(rules?: AclRuleEntity[]): Promise<string> {
    const enabledRules = rules ?? await aclRepository.findEnabled();
    const rateLimitRules = enabledRules.filter(r => r.conditionField === 'rate_limit');

    let config = `# ACL Rate Limit Zones - Auto-generated by Nginx Love UI
# Do not edit manually - Changes will be overwritten
# Generated at: ${new Date().toISOString()}
\n`;

    if (rateLimitRules.length === 0) {
      config += `# No rate limit rules configured\n`;
      return config;
    }

    config += `# ===== Rate Limit Zones (http context) =====\n\n`;
    ...
    for (const rule of rateLimitRules) {
      ...
      const zoneName = this.buildRateLimitZoneName(rule);
      const rate = rule.conditionValue.trim();

      config += `# ${rule.name}\n`;
      config += `limit_req_zone ${this.RATE_LIMIT_KEY} zone=${zoneName}:${this.RATE_LIMIT_ZONE_SIZE} rate=${rate};\n\n`;
    }
  ...
  /**
   * Build a safe rate limit zone name
   */
  private buildRateLimitZoneName(rule: AclRuleEntity): string {
    const base = rule.id || rule.name || 'rule';
    return `acl_rl_${base}`.replace(/[^a-zA-Z0-9_]/g, '_');
  }

  /**
   * Map ACL action to limit_req status code
   */
  private getRateLimitStatus(rules: AclRuleEntity[]): number | null {
    const hasChallenge = rules.some(rule => rule.action === 'challenge');
    const hasDeny = rules.some(rule => rule.action === 'deny');
    if (hasChallenge) {
      return 429;
    }
    if (hasDeny) {
  ...
  /**
   * Write rate limit config to Nginx configuration file
   */
  async writeRateLimitConfig(config: string): Promise<void> {
    try {
      await fs.writeFile(this.ACL_RATE_LIMIT_CONFIG_FILE, config, 'utf8');
      logger.info(`ACL rate limit config written to ${this.ACL_RATE_LIMIT_CONFIG_FILE}`);
    } catch (error) {
      logger.error('Failed to write ACL rate limit config:', error);
      throw error;
    }
  }
  ...
  async applyAclRules(): Promise<AclNginxResult> {
    try {
      ...
      const config = await this.generateAclConfig(rules);
      const rateLimitConfig = await this.generateRateLimitConfig(rules);

      // 2. Write to file
      logger.info('Writing ACL config to Nginx...');
      await this.writeAclConfig(config);
      await this.writeRateLimitConfig(rateLimitConfig);
...
\n# No rules configured yet\n`;
      ...
      try {
        await fs.access(this.ACL_RATE_LIMIT_CONFIG_FILE);
        logger.info('ACL rate limit config file already exists');
      } catch {
        const emptyConfig = `# ACL Rate Limit Zones - Nginx Love UI
# This file will be populated with rate limit zones
\n# No rate limit rules configured yet\n`;

        await this.writeRateLimitConfig(emptyConfig);
        logger.info('ACL rate limit config file initialized');
      }
    } catch (error) {
